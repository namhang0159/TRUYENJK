  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-[#0a0a0a] pb-24">
      ...
      <div className="relative overflow-hidden bg-white dark:bg-black border-b border-zinc-200 dark:border-white/10 pt-16 pb-12">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-500/10 via-transparent to-transparent dark:from-indigo-900/30" />
        <div className="container max-w-7xl mx-auto px-4 relative z-10 flex flex-col items-center text-center">
          <motion.div
            initial={{ scale: 0.5, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ type: "spring", stiffness: 200, damping: 20 }}
            className="w-16 h-16 rounded-2xl bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center mb-6 shadow-xl shadow-indigo-500/10"
          >
          ...
          >
            Khám Phá <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-cyan-500">Thế Giới Truyện</span>
          </motion.h1>
          <motion.p 
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
          ...
          >
            Hàng ngàn tác phẩm độc quyền chất lượng cao đang chờ đón bạn. Lọc và tìm kiếm theo sở thích cá nhân.
          </motion.p>
        </div>
      </div>
      ...
      <div className="container max-w-7xl mx-auto px-4 py-8 space-y-10">
        ...
        {isLoading ? (
          ...
          >
            ...
            <Button 
              onClick={() => setFilters({ category_slug: "all", tag_slug: "all", status: "all", sort_by: "created_at", page: 1 })}
              className="mt-6 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white"
            >
              Xóa bộ lọc
            </Button>
          </motion.div>
